...
class StreamedAudioResult:
    ...
    def __init__(
        ...
        # call `_finish_turn` again for these — doing so could prematurely
        # finish a later turn's tracing span that started after them.
    ...
    async def _turn_done(self):
        if self._text_buffer:
            ...
            # Finish turn state synchronously so a follow-up `_add_text` for the next
            # turn doesn't race the dispatcher and skip its `turn_started`. The
            # synthesized `turn_ended` flows through the dispatcher to stay ordered
            # with any in-flight audio segments; a counter tells the dispatcher this
            # event's `_finish_turn` is already done so it doesn't clobber a span
            # that belongs to a later turn started in the meantime.
            self._finish_turn()
            ...
            if self._dispatcher_task is None:
                self._dispatcher_task = asyncio.create_task(self._dispatch_audio())
            # Wait for the dispatcher to drain this synthesized `turn_ended` so the
            # next turn's `turn_started` cannot precede it on `self._queue`.
