...
Realtime function tools receive the JSON schema generated by the SDK, but the Realtime API does not enforce strict function-tool schemas in the same way as the Responses API. If a tool depends on deeply nested object arguments, especially through MCP tools, the model may still produce flattened or otherwise invalid arguments.

Prefer simple, flat argument shapes for realtime tools when possible. If you need richer input, pass a JSON string and validate it inside the tool, or validate the final parsed payload before applying side effects.
